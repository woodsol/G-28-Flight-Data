...
void draw() {
  ...
  // Draw Departure/Arrival buttons
  drawToggle("Departure", 340, 700, isDeparture);
  drawToggle("Arrival", 620, 700, !isDeparture);
...
void mousePressed() {
  ...
  if (mouseY > 700 && mouseY < 760) {
    if (mouseX > 340 && mouseX < 580) {
      isDeparture = true;
    }
    else if (mouseX > 620 && mouseX < 860) {
      isDeparture = false;
    }
  }
  ...
  if (mouseX > 450 && mouseX < 750 && mouseY > 800 && mouseY < 900) {
    ...
    println("Flight Type: " + (isDeparture ? "Departure" : "Arrival"));
    println("Transitioning to Flight Results Screen...");
  }
}
