  int dotX1 = 385; // X-coordinate for the first dot (Boise)
 int dotY1 = 240; // Y-coordinate for the first dot (Boise)
 ...
 void setup() {
   ...
   Idaho = loadImage("Idaho.jpg"); // Load the image for the background
 }
 ...
 void draw() {
   ...
   // Draw label for Boise
   fill(0,0,0);
   textSize(20);
   textAlign(CENTER, CENTER);
   text("Boise", dotX1  - 30, dotY1 + dotSize1 / 2 - 30);
 ...
 void mousePressed() {
   // Check if the mouse is clicked on the first dot (Boise)
   if (dist(mouseX, mouseY, dotX1, dotY1) < dotSize1 / 2) {
     fill(0, 0, 255); // Change color to blue if clicked on Boise
