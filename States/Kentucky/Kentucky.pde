int dotX1 = 300; // X-coordinate for the first dot (Louisville)
int dotY1 = 350; // Y-coordinate for the first dot (Louisville)
int dotSize1 = 20; // Size of the first dot

PImage Kentucky; // Declare an image object for the background

void setup() {
  size(600, 600); // Set the canvas size
  background(255); // Set the background color to white
  Kentucky = loadImage("Kentucky.jpg"); // Load the image for the background
}

void draw() {
  image(Kentucky, 0, 0, width, height); // Display the image as the background
  ...
  drawLabel("Louisville", dotX1, dotY1);
}
