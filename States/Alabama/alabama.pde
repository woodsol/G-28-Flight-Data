int dotX1 = 200; // X-coordinate for the first dot (Nashville)
int dotY1 = 430; // Y-coordinate for the first dot (Nashville)
int dotSize1 = 20; // Size of the first dot
int dotX2 = 290; // X-coordinate for the second dot (Memphis)
...
void setup() {
  ...
  alabama = loadImage("alabama.jpg"); // Load the image for the background
}

void draw() {
  image(alabama, 50, 50, width-100, height-100); // Display the image as the background
  ...
  text("Mobile Regl", dotX1, dotY1 + dotSize1 / 2 + 20);
  ...
  textSize(20);
  textAlign(CENTER, CENTER);
  text("Birmingham", dotX2, dotY2 + dotSize2 / 2 + 20);
  ...
  textSize(20);
  textAlign(CENTER, CENTER);
  text("Huntsville", dotX3, dotY3 + dotSize3 / 2 + 20);
}
