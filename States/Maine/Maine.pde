int dotX1 = 270; // X-coordinate for the first dot (Portland)
int dotY1 = 180; // Y-coordinate for the first dot (Portland)
int dotSize1 = 20; // Size of the first dot

PImage maine; // Declare an image object for the background

void setup() {
  size(600, 600); // Set the canvas size
  background(255); // Set the background color to white
  maine = loadImage("Maine.jpg"); // Load the image for the background (Maine image)
}

void draw() {
  image(maine, 0, 0, width, height); // Display the image as the background
  ...
  drawLabel("Portland", dotX1, dotY1);
}
