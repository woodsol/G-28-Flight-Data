int dotX1 = 370; // X-coordinate for the first dot (Nashville)
int dotY1 = 190; // Y-coordinate for the first dot (Nashville)
int dotSize1 = 20; // Size of the first dot
int dotX2 = 150; // X-coordinate for the second dot (Memphis)
int dotY2 = 510; // Y-coordinate for the second dot (Memphis)
int dotSize2 = 20; // Size of the second dot
PImage Colerado; // Declare an image object for the background

void setup() {
  size(600, 600); // Set the canvas sizexa
  background(255); // Set the background color to white
  Colerado = loadImage("colerado.jpg"); // Load the image for the background
}

void draw() {
  image(Colerado, 50, 50, width-100, height-100); // Display the image as the background
  ...
  text("Denver", dotX1, dotY1 + dotSize1 / 2 + 20);
  ...
  textSize(20);
  textAlign(CENTER, CENTER);
  text("Durango", dotX2, dotY2 + dotSize2 / 2 + 20);
}  
