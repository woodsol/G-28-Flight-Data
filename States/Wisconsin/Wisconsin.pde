int dotX1 = 450; // X-coordinate for the first dot (Nashville)
int dotY1 = 460; // Y-coordinate for the first dot (Nashville)
int dotSize1 = 20; // Size of the first dot
int dotX2 = 330; // X-coordinate for the second dot (Memphis)
int dotY2 = 470; // Y-coordinate for the second dot (Memphis)
int dotSize2 = 20; // Size of the second dot
PImage Wisconsin; // Declare an image object for the background

void setup() {
  size(600, 600); // Set the canvas size
  background(255); // Set the background color to white
  Wisconsin = loadImage("Wisconsin.jpg"); // Load the image for the background
}

void draw() {
  image(Wisconsin, 0, 0, width, height); // Display the image as the background
  ...
  text("Milwaukee", dotX1, dotY1 + dotSize1 / 2 + 20);
  ...
  textSize(20);
  textAlign(CENTER, CENTER);
  text("Madison", dotX2, dotY2 + dotSize2 / 2 + 20);
}
