...
void setup() {
  ...
  println("Loading flight data...");
  loadFlightData("flights.csv");
  println("Flight data loaded and processed!");
  ...
  println("Maximum density: " + maxDensity);
  
  // Draw the heatmap once (no need to redraw for static data)
  drawHeatmap();
}

void draw() {
  // Nothing needed here for static visualization
  // If you want interactive features, you can add them here
}
...
void loadFlightData(String filename) {
  ...
  for (int i = 0; i < totalRows; i += batchSize) {
    ...
    // Show progress
    if (i % 50000 == 0) {
      println("Processed " + i + " of " + totalRows + " flights...");
    }
  }
}
...
void drawLegend() {
  ...
  textSize(24);
  textAlign(CENTER);
  text("Flight Density Heatmap (1,000,000 Flights)", width/2, 30);
